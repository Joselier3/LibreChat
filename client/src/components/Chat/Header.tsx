    ...
    <div className="sticky top-0 z-10 flex h-14 w-full items-center justify-between bg-white p-2 font-semibold dark:bg-gray-800 dark:text-white">
      <div className="hide-scrollbar flex w-full items-center justify-between gap-2 overflow-x-auto">
        <div className="flex items-center gap-2">
          ...
          <SelectWorkspaces />
          {selectedWorkspace?.connections.length && interfaceConfig.endpointsMenu === true ? (
            <EndpointsMenu />
          ) : null}
          {/* {modelSpecs.length > 0 && <ModelSpecsMenu modelSpecs={modelSpecs} />} */}
          {/* muestra la configuracion prestablecida de la IA y permite cambiar de asistente  */}
          {selectedWorkspace?.connections.length ? (
            <HeaderOptions interfaceConfig={interfaceConfig} />
          ) : null}
          {selectedWorkspace?.connections.length && interfaceConfig.presets === true ? (
            <PresetsMenu />
          ) : null}
          {selectedWorkspace?.connections.length && hasAccessToBookmarks === true ? (
            <BookmarkMenu />
          ) : null}
          {selectedWorkspace?.connections.length && hasAccessToMultiConvo === true ? (
            <AddMultiConvo />
          ) : null}
          {isSmallScreen && (
            <ExportAndShareMenu
