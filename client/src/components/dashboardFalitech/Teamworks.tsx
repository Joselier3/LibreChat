    ...
    <section className=" w-full  p-4">
      <div className="mb-8 overflow-hidden rounded-md border shadow">
        ...
        <form onSubmit={sendInvitation} name="create-workspace" className="bg-gray-50 p-4">
          ...
          <input type="email" onChange={(e) => setInvitedEmail(e.target.value)} name='email' className="w-full rounded-lg border p-2 focus:outline-[#0084ff]/60 " />
          <button type='submit' disabled={!invitedEmail || invitationLoading} className="mt-3 rounded-md border bg-[#2791d1]/80 p-2 px-5 text-white transition-all hover:bg-[#2791d1] flex items-center gap-2 disabled:cursor-not-allowed disabled:bg-gray-200">
      ...
      {
        invitationLink && (
          <div className="mb-8 overflow-hidden rounded-md border shadow flex flex-col">
            <div className='flex items-center justify-between gap-1 p-2'>
              <h2 className="">Enlace de invitación</h2>
              <button onClick={()=>setInvitationLink('')} className="mt-2 flex items-center gap-1 rounded-md border border-[#dc3545] p-3  text-[#dc3545] transition-all hover:bg-[#dc3545] hover:text-white disabled:cursor-not-allowed disabled:bg-gray-200 disabled:border-gray-100 disabled:text-gray-400" title='Eliminar area de trabajo' >
                <IoClose />
              </button>
            </div>
            <div className="bg-gray-50 p-4 " >
              <a href={invitationLink} target='_blank' rel="noreferrer" className='hover:text-[#0084ff]/50 hover:underline'>{invitationLink}</a>
            </div>
          </div>
        )
      }
      {isLoading && <TableEsqueleton rows={3} />}
      {
        ...
        <div className="overflow-hidden rounded-md border shadow">
          <h2 className="p-4">
            Miembros del espacio de trabajo ({members.length})
          </h2>
